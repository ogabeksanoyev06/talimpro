...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "py-8 bg-gray-100 dark:bg-transparent" }, _attrs))}><div class="container"><div class="space-y-2"><h1 class="text-2xl sm:text-4xl font-semibold tracking-tight">Abiturientlar va o&#39;quvchilar uchun test topshirish tizimi</h1><p class="text-base sm:text-lg text-muted-foreground">DTM talablari asosida tuzilgan 40 000 ta test savollaridan foydalangan holda test topshirish imkoni</p></div><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-8"><!--[-->`);
      ssrRenderList(unref(testTypes), (item) => {
        _push(`<div class="relative rounded-xl bg-card border-b border-border dark:border-white p-4 md:p-6 flex flex-col cursor-pointer transition-300 hover:shadow-card h-full gap-2 md:gap-4"><div class="flex-grow flex flex-col gap-2"><h3 class="text-xl font-semibold w-full">${ssrInterpolate(item.title)}</h3><span class="text-justify sm:text-start">${ssrInterpolate(item.short_description)}</span></div><div class="flex flex-col gap-2 sm:flex-row sm:justify-between">`);
        ...
        if (!item.is_free && !item.bought) {
          _push(ssrRenderComponent(_sfc_main$1, {
            ...
            onClick: ($event) => unref(testStore).buyExams(item.id, "test")
          }, {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                if (unref(loading) && item.id === unref(testId)) {
...
//# sourceMappingURL=choose-test-BMFiTwvF.mjs.map
